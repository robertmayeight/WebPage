...
slide.open("GET","module_6.svg",false);
...
TweenMax.to([diagram1S1,diagram1S1copy],0,{rotation:30, transformOrigin:"0 20"});

var diag1branch1CurrentArray = [diagram1Wire1copy,diagram1S1copy,diagram1Wire2copy,diagram1R1copy,diagram1Wire3copy,diagram1R2copy,diagram1Wire4copy,path6501copy,diagram1Wire5copy];
TweenMax.to(diag1branch1CurrentArray, 0, {stroke:'black'})
var directCurrentTl = new TimelineMax({paused:true, repeat:-1})
directCurrentTl.timeScale(.5);
directCurrentTl
...
var alternatingCurrentTl = new TimelineMax({paused:true, repeat:-1})
alternatingCurrentTl.timeScale(.5);
alternatingCurrentTl
.to(diag1branch1CurrentArray, 0, {strokeDasharray:"2,6", ease:Linear.easeNone, strokeWidth:2.5})
.to(diag1branch1CurrentArray, 0.08, {strokeDashoffset:"-=16", repeat:-1, ease:Linear.easeNone,yoyo:true})

// .to(diag1branch1CurrentArray, 0, {strokeDasharray:"2,6", ease:Linear.easeNone, strokeWidth:2.5})
// .to(diag1branch1CurrentArray, 0.1, {strokeDashoffset:"-=8", repeat:0, ease:Linear.easeNone,yoyo:false})
// .to(diag1branch1CurrentArray, 0, {strokeDasharray:"6,2", ease:Linear.easeNone, strokeWidth:2.5})
// .to(diag1branch1CurrentArray, 0.1, {strokeDashoffset:"+=8", repeat:0, ease:Linear.easeNone,yoyo:false})

var slideTl = new TimelineMax({paused:true});
slideTl
.to(headingText,1,{autoAlpha:0, delay:1})
.from(diagram1, 1, {autoAlpha:0, delay:0})
.set(batteryBorder, {stroke:'red'})
.staggerFromTo([batteryBorder], 2, {drawSVG:'0% 0%'}, {drawSVG: '0% 100%', ease: Power0.easeNone, delay:3})
.to(batteryBorder, 1, {autoAlpha:0, delay:1})

.to([diagram1S1,diagram1S1copy],0,{rotation:0, transformOrigin:"0 20", delay:0})
.add(function(){directCurrentTl.play()})
.to(diag1branch1CurrentArray, 1, {autoAlpha:1})

.to(diagram1, 1, {autoAlpha:0, delay:3})
.from(dcIllustration, 1, {autoAlpha:0, delay:0})

//Move DC over to give ac room.
.to(dcIllustration, 1, {x:'-=212', y:'+=100', scaleX:.8, scaleY:.8, ease:Linear.easeNone, delay:1})

.to(acIllustration, 0, {x:'+=212', y:'-=100', scaleX:.84, scaleY:.84, ease:Linear.easeNone, delay:0})
.from(acIllustration, 1, {autoAlpha:0})

//Draw Sinewave
.staggerFromTo([sineWave1], 5, {drawSVG:'0% 0%'}, {drawSVG: '0% 100%', ease: Power0.easeNone, delay:-1})
.to(sineWave1, 1, {autoAlpha:0})
.staggerFromTo([sineWave1], 0, {drawSVG:'0% 100%'}, {drawSVG: '0% 0%', ease: Power0.easeNone, immediateRender:false, delay:1})
.to(sineWave1, 1, {autoAlpha:1})
.staggerFromTo([sineWave1], 4, {drawSVG:'0% 0%'}, {drawSVG: '0% 6.2%', ease: Power0.easeNone, immediateRender:false, delay:1})
.staggerFromTo([sineWave1], 4, {drawSVG:'0% 6.2%'}, {drawSVG: '0% 12.35%', ease: Power0.easeNone, immediateRender:false, delay:1})
.staggerFromTo([sineWave1], 4, {drawSVG:'0% 12.35%'}, {drawSVG: '0% 18.7%', ease: Power0.easeNone, immediateRender:false, delay:10})
.staggerFromTo([sineWave1], 4, {drawSVG:'0% 18.7%'}, {drawSVG: '0% 25%', ease: Power0.easeNone, immediateRender:false, delay:3})
.staggerFromTo([sineWave1], 4, {drawSVG:'0% 25%'}, {drawSVG: '0% 100%', ease: Power0.easeNone, immediateRender:false, delay:3})
.from(sixtyHertz, 1, {autoAlpha:0})

//Close out ac and dc charts.
.to([dcIllustration,acIllustration,sixtyHertz], 1, {autoAlpha:0, delay:4})

.to([diagram1S1,diagram1S1copy],0,{rotation:0, transformOrigin:"0 20", delay:0})
.add(function(){alternatingCurrentTl.play()})
.to(diag1branch1CurrentArray, 1, {autoAlpha:1})
.set([diagram1BatterySymbol_noCurrent,minus1_hide,plus1_hide,diagram1B1V], {autoAlpha:0})
.from([acPowerSymbol,diagram1ACV], .1, {autoAlpha:0})
.to(diagram1, 1, {autoAlpha:1, delay:0})

//Show AC waveform again
.set(acIllustration, {x:-150, y:-100, scaleX:1.5, scaleY:1.5, ease:Linear.easeNone, delay:0})
.to(diagram1, 1, {autoAlpha:0, delay:9})
.to(acIllustration, 1, {autoAlpha:1, delay:0})

.from(t1, 1, {autoAlpha:0, delay:30})
.to(t1, 1, {autoAlpha:0, delay:2})

.from(t2, 1, {autoAlpha:0, delay:0})
.to(t2, 1, {autoAlpha:0, delay:2})

.from(t3, 1, {autoAlpha:0, delay:0})
.to(t3, 1, {autoAlpha:0, delay:2})

.from(t4, 1, {autoAlpha:0, delay:0})
.to(t4, 1, {autoAlpha:0, delay:2})

.from(t5, 1, {autoAlpha:0, delay:0})
.to(t5, 1, {autoAlpha:0, delay:.5})

.from(t6, 1, {autoAlpha:0, delay:0})
.to(t6, 1, {autoAlpha:0, delay:.5})

.from(t7, 1, {autoAlpha:0, delay:0})
.to(t7, 1, {autoAlpha:0, delay:.5})
